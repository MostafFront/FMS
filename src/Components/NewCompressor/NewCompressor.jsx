...
export default function NewCompressor({ data, setData }) {

    const [formData, setFormData] = useState({ name: '', id: '', station: ''});
        ...
        <form onSubmit={handleSubmit} class=" p-4 mx-auto w-[60%] max-h-full -translate-y-2/4 -translate-x-2/4 absolute top-2/4 left-2/4 bg-[#FFFFFF] rounded-md">
            <div className='flex items-center justify-between border-b-2 border-[#B2BEC3] p-2 mb-5'>
                <div className='flex gap-2 '>
                    <i className="fa-solid fa-compress text-base"></i>
                    <p className='font-medium text-xs'>{location.state && location.state.index !== undefined ? 'Edit Compressor' : 'Add New Compressor'}</p>
                </div>
                <button type="button" onClick={handleCancel}><i class="fa-solid fa-xmark"></i></button>
            </div>
            <div className='w-full h-full flex justify-end items-center pr-5 pl-5 flex-wrap gap-2'>
                <div class="w-[50%]">
