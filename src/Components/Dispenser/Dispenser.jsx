...
export default function Dispenser({ data, setData }) {
    ...
    const handleEdit = (index) => {
        ...
        navigate('/NewDispenser', { state: { station: stationToEdit, index } });
    };
            ...
            <div className='h-full pt-[10%] w-[95%] absolute right-0 bottom-0 justify-center items-start flex bg-[#F9F5F3]'>
                <div className='flex flex-col w-[80%] min-h-full justify-start items-end'>
                    <div className='flex-col-reverse flex mb-6'>
                        ...
                        >
                            <option value="Station Filter">Station Filter</option>
                    ...
                    <div className="relative shadow-md sm:rounded-lg w-full overflow-auto max-h-96 scrollbar-thin scrollbar-thumb-gray-500 scrollbar-track-gray-200">
                        <table className="w-full text-sm text-left rtl:text-right bg-[#FFFFFF]">
                            <thead className="text-xs text-[#707070] border-b border-[#B2BEC3]">
                                <tr>
                                    <th scope="col" className="px-6 py-3">Dispenser Name</th>
                            ...
                            <tbody>
                                {filteredData.length === 0 ? (
                                    <tr>
                                        <td colSpan="7" className="px-6 py-4 text-center text-gray-500">There is no info</td>
                                    </tr>
                                ) : (
                                    filteredData.map((item, index) => (
                                        <tr key={index} className="border-b text-[#707070]">
                                            ...
                                            <td className="px-6 py-4 truncate">{item.line}</td>
                                            <td className="px-6 py-4 truncate">{item.order}</td>
                                            <td className="px-6 py-4 truncate">{item.position}</td>
                                            <td className='flex justify-end items-center px-6 py-4'>
                                                <button onClick={() => handleEdit(index)}>
                                                    <i className="fa-regular fa-pen-to-square text-sm p-2 mr-4 text-[#2C3E50] bg-[#3D516133] rounded-md cursor-pointer"></i>
                                                </button>
                                                <button onClick={() => handleDelete(index)}>
                                                    <i className="fa-regular fa-trash-can text-sm p-2 text-[#FF7F5C] bg-[#FF7F5C33] rounded-md cursor-pointer"></i>
                                                </button>
                                            </td>
                                        </tr>
                                    ))
                    ...
                    <div className='flex-col-reverse flex mt-6'>
                        <Link to={'/NewDispenser'} className="drop-shadow-xl text-white bg-[#FF7F5C] hover:bg-[#f56b44] focus:ring-4 focus:ring-[#FF7F5C] font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 focus:outline-none">
